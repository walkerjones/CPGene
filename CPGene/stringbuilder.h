...
public:
	...
	void setCharAt(int iGeneToSwap, char cGene)
	{
		privateString[iGeneToSwap] = cGene;
	}
	...
	StringBuilder *replace(std::size_t start, std::size_t end, const std::string &nestring)
	{
		privateString.replace(start, end - start, nestring);
		return this;
	}
